...
<template>
  <dialog
    ref="dialogEl"
    class="m-0 h-dvh max-h-none w-screen max-w-none bg-black/95 p-0"
    @close="emit('close')"
    @keydown="onKeydown"
    ...
    <div v-if="open && current" class="relative flex h-full flex-col">
      ...
      <div class="relative flex min-h-0 flex-1 items-center justify-center px-4 pb-4 md:px-20">
        ...
        <template v-if="items.length > 1">
          <button type="button" class="lb-nav left-3 md:left-6" aria-label="Foto sebelumnya" @click="prev">
            <IconGlyph name="chevron-left" class="h-5 w-5" />
          </button>
          <button type="button" class="lb-nav right-3 md:right-6" aria-label="Foto berikutnya" @click="next">
            <IconGlyph name="chevron-right" class="h-5 w-5" />
          </button>
        </template>
      </div>

      <p class="px-6 pb-5 text-center text-sm text-white/75">{{ current.alt }}</p>
    </div>
  </dialog>
</template>
